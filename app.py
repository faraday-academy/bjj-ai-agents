...
def chat_with_coach(message: str, use_tools: bool = False) -> str:
    """Chat with the BJJ coach agent"""
    try:
        if use_tools:
            return run_coach_agent_with_tools(message)
        else:
            return run_coach_agent(message)
    except Exception as e:
        return f"Error: {str(e)}"


def get_game_plan(tournament_info: str, use_rag: bool = False) -> str:
    """Generate a game plan for a tournament"""
    try:
        if use_rag:
...
def track_progress(technique: str, level: str, notes: str = "") -> str:
    """Track student progress"""
    try:
        from app.agents.coach_agent import track_student_progress

        return track_student_progress(technique, level, notes)
    except Exception as e:
        return f"Error: {str(e)}"
...
with gr.Blocks(title="BJJ AI Agents", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("🤖 Coach Chat"):
            ...
            coach_button.click(
                fn=chat_with_coach,
                inputs=[coach_input, use_tools_checkbox],
            ...
            with gr.Row():
                with gr.Column():
                    game_plan_input = gr.Textbox(
                        ...
                        placeholder="e.g., I'm competing in the blue belt division, 155 lbs, against aggressive opponents...",
                        lines=5,
                    )
        ...
        with gr.TabItem("📊 Progress Tracking"):
            ...
            track_button.click(
                fn=track_progress,
                inputs=[technique_input, level_input, notes_input],
